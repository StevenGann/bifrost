...
- **Observability (v0.2):** Prometheus `/metrics` — request counts, token in/out,
  estimated cost (cache-miss rates, peak/off-peak aware), latency histogram, and
  error counts, labeled by model and app (`X-Bifrost-App` header).
...
### 2. Usage & cost observability — ✅ shipped (v0.2)

See the current-state bullet. Next steps: a Grafana dashboard (the homelab already
runs the stack) and Prometheus scraping of `bifrost.lab:11434/metrics`.
